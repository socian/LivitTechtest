...
The elements that builts up the game are Level, Steps, Items and Quests. A level contains several steps. A single step can contain several items and quests. Several rules can be defined in the XML before a player can move from one step to an other.
...
```
This is the first step in the demonstration. The "clearinventory" attribute of the step node which is set to true means that the inventory should always be cleared if the player get into this step.
